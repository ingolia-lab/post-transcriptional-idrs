...
for aa in tqdm(aas):
    filtered[aa] = filtered['seq'].str.count(f'(?=({aa}))') 

filtered[aas] = filtered[aas]/50

print(f'are all aas normalized?: {all(filtered[aas].sum(axis=1)- 1) < 1e-10}')


x_train, x_test, y_train, y_test = train_test_split(filtered[aas], filtered['repressor'], 
                                                    test_size=0.20, random_state=18)

with open('x_train_compo_single.pkl', 'wb') as file:
    pickle.dump(x_train, file)

with open('x_test_compo_single.pkl', 'wb') as file:
    pickle.dump(x_test, file)

with open('y_train_compo_single.pkl', 'wb') as file:
    pickle.dump(y_train, file)

with open('y_test_compo_single.pkl', 'wb') as file:
