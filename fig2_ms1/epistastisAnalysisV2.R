...
ccr4_plot = ggplot(ccr4_fit, aes(x=YFP_mean,y=ccr4_avg_activity))+
  geom_pointdensity(size=0.15, show.legend = FALSE) +
  geom_point(data = subset(ccr4_fit,YFP_mean<=-1 & ccr4_z_adj <= 0.05),
             mapping = aes(x=YFP_mean,y=ccr4_avg_activity),size=0.3,color='#e31a1c') + 
...
pop2_plot = ggplot(pop2_fit, aes(x=YFP_mean,y=pop2_avg_activity))+
  geom_pointdensity(size=0.15, show.legend = FALSE) +
  geom_point(data = subset(pop2_fit,YFP_mean<=-1 & pop2_z_adj <= 0.05),
             mapping = aes(x=YFP_mean,y=pop2_avg_activity),size=0.3,color='#e31a1c') + 
...
dcp2_plot = ggplot(dcp2_fit, aes(x=YFP_mean,y=dcp2_avg_activity))+
  geom_pointdensity(size=0.15, show.legend = FALSE) +
  geom_point(data = subset(dcp2_fit,YFP_mean<=-1 & dcp2_z_adj <= 0.05),
             mapping = aes(x=YFP_mean,y=dcp2_avg_activity),size=0.3,color='#e31a1c') + 
...
supp_maid = grid.arrange(ccr4_plot,dcp2_plot,ncol=2)



setwd("~/post-transcriptional-idrs/fig2_ms1/")
if(!file.exists('pop2_maid.pdf')){
  ggsave('pop2_maid.pdf', pop2_plot,height=5, width=20, units='cm')
}
...
#load the individual replicate scores and merge with the dataset;
setwd("~/post-transcriptional-idrs/processed_scores/")
indiv_scores = read.csv("wt_yfp_irfp_scores.csv")
colnames(indiv_scores)[2] = 'Peptide'
edc1_zoom = merge(edc1_zoom,indiv_scores,by='Peptide')
...
edc1_epi <- ggplot(edc1_zoom, aes(x = start, y = YFP_mean.x)) +
  ...
  geom_point(aes(x=start+24,y=activity_score_r1),size=0.5) + 
  geom_point(aes(x=start+24,y=activity_score_r2),size=0.5) + 
  geom_point(aes(x=start+24,y=ccr4_loess_rep1),size=0.5, color=cols[1]) +
  geom_point(aes(x=start+24,y=ccr4_loess_rep2),size=0.5,color=cols[1]) + 
  geom_point(aes(x=start+24,y=pop2_loess_rep1),size=0.5,color=cols[2]) + 
  geom_point(aes(x=start+24,y=pop2_loess_rep2),size=0.5,color=cols[2]) + 
  geom_point(aes(x=start+24,y=dcp2_loess_rep1),size=0.5,color=cols[3]) + 
  geom_point(aes(x=start+24,y=dcp2_loess_rep2),size=0.5,color=cols[3]) + 
  ylim(-2,1) + 
...
setwd("~/post-transcriptional-idrs/fig2_ms1/edc1_zoom/")
