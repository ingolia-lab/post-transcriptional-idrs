...
pho92_plot <- ggplot(pho92_df, aes(x = start, y = cutoff_activity)) +
  ...
  geom_point(aes(x=start+2,y=activity_score_r1),size=1) + 
  geom_point(aes(x=start+2,y=activity_score_r2),size=1) + 
...
if(!file.exists('pho_caf_hmms.pdf')){
  composite = grid.arrange(pho92_plot, caf20_plot,ncol=2)
  ggsave('pho_caf_hmms.pdf',composite, width = 17, height = 4.5, units = 'in')
}


###for tis11 and Eap1, fig 4
...
tis11_plot <- ggplot(tis11_df, aes(x = start, y = cutoff_activity)) +
  ...
  geom_line(aes(x=start+2,y=cutoff_activity), size=1,color='#b2182b') + 
  geom_line(aes(x=start+2,y=hmm_act), size=1.5,color='#67a9cf') + 
  geom_point(aes(x=start+2,y=activity_score_r1),size=0.5) + 
  geom_point(aes(x=start+2,y=activity_score_r2),size=0.5) + 
  xlab("Position") +ylab("Activity Score") + ylim(-2.1,1.3) + theme_classic() + 
  theme(panel.grid = element_blank(),
        axis.line = element_line(color = "black"),
        axis.ticks = element_line(color = "black"),
...
eap1_m1_plot <- ggplot(eap1_m1_df, aes(x = start, y = cutoff_activity)) +
  ...
  geom_point(aes(x=start+2,y=activity_score_r1),size=0.5) + 
  geom_point(aes(x=start+2,y=activity_score_r2),size=0.5) + 
  xlab("Position") +ylab("Activity Score") + ylim(-1.2,1.43) + theme_classic() + 
  theme(panel.grid = element_blank(),
        axis.line = element_line(color = "black"),
        axis.ticks = element_line(color = "black"),
        legend.position = "none",
...
setwd("~/post-transcriptional-idrs/fig4_ms1/eap1_motif_hmms/")
